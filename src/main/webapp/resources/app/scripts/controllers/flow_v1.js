...
app.controller('FlowControllerV1', ['$scope', '$stateParams', 'FlowServiceV1', 'ngDialog',
  ...
  function ($scope, $stateParams, FlowService, ngDialog,
    ...
    $scope.saveRule = function () {
      if (!FlowService.checkRuleValid($scope.currentRule.rule)) {
        return;
      }
    ...
    $scope.deleteRule = function (rule) {
      ...
      $scope.confirmDialog = {
        ...
        attention: '资源名: ' + rule.rule.resource + ', 流控应用: ' + rule.rule.limitApp
          + ', 阈值类型: ' + (rule.rule.grade == 0 ? '线程数' : 'QPS') + ', 阈值: ' + rule.rule.count,
        confirmBtnText: '删除',
      };
